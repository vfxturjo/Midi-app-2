...
function handleMoveStart(event) {
  ...
  const activeMoveParentClass =
    handleLeft || handleRight ? "layout-row" : "layout-column";

  const sizeKeyPrefix = "offset"; // all the same?
  activeMoveSizeKey =
    activeMoveParentClass == "layout-column"
  ...
  if (
    ...
    for (let [index, node] of containerChildren.entries()) {
      node.style.flexBasis =
        Math.round((node_size_new[index] / size_sum) * 100) + "%";
    }
  }
...
{:else}
  <!-- layout cell -->
  <div class="layout-cell" style="{styleStr}">
    <div class="top">
      <div class="frame left" on:mousedown="{handleMoveStart}"></div>
      <div class="frame center" on:mousedown="{handleMoveStart}"></div>
      <div class="frame right" on:mousedown="{handleMoveStart}"></div>
    </div>

    <div class="middle">
      <div class="frame left" on:mousedown="{handleMoveStart}"></div>

      <div class:center="{true}" style="{style}" {...$$restProps}>
        <slot />
      </div>
